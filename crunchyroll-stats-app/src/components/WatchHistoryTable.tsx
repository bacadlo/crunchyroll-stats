...
type SortField = 'animeTitle' | 'dateWatched' | 'completionPercent';
...
export const WatchHistoryTable: React.FC<WatchHistoryTableProps> = ({ data, searchQuery }) => {
  const [sortField, setSortField] = useState<SortField>('dateWatched');
    ...
    if (searchQuery) {
      filtered = data.filter(
        (item) =>
          ...
          item.episodeName.toLowerCase().includes(searchQuery.toLowerCase())
      );
    }
    ...
    return [...filtered].sort((a, b) => {
      ...
      switch (sortField) {
        case 'animeTitle':
          ...
          break;
        case 'dateWatched':
          comparison = new Date(a.dateWatched).getTime() - new Date(b.dateWatched).getTime();
          break;
          ...
          <thead>
            <tr className="border-b border-gray-200 dark:border-gray-700">
              ...
              <th className="px-4 py-3 text-left">
                ...
                >
                  Anime Title
                  <SortIcon field="animeTitle" sortField={sortField} sortOrder={sortOrder} />
                </button>
              </th>
              <th className="px-4 py-3 text-left font-semibold text-gray-900 dark:text-gray-100">Episode</th>
              <th className="px-4 py-3 text-left">
                <button
                  onClick={() => handleSort('dateWatched')}
                  className="flex items-center gap-2 font-semibold text-gray-900 dark:text-gray-100 hover:text-primary-600 transition-colors"
                >
                  Date Watched
                  <SortIcon field="dateWatched" sortField={sortField} sortOrder={sortOrder} />
                </button>
              </th>
              <th className="px-4 py-3 text-left">
                <button
                ...
                >
                  Completion
                  <SortIcon field="completionPercent" sortField={sortField} sortOrder={sortOrder} />
                </button>
              </th>
            ...
            {paginatedData.length === 0 ? (
              ...
              paginatedData.map((item) => (
                ...
                >
                  ...
                  <td className="px-4 py-4">
                    <div className="font-medium text-gray-900 dark:text-white">{item.animeTitle}</div>
                  </td>
                  <td className="px-4 py-4">
                    <div className="text-sm text-gray-600 dark:text-gray-400">
                      {item.episodeNumber ? `Ep ${item.episodeNumber}` : 'N/A'}
                    </div>
                  ...
                  <td className="px-4 py-4">
                    <span
                      className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${getCompletionColor(
